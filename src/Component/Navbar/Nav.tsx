...
const Navbar = ({ isAdmin }:any) => { // Accept a prop "isAdmin" to determine the user role
  ...
  return (
    <div>
      <div className="top-bar">
        {isMobile && (
          <i
            className={`nav__toggle fa ${isMenuOpen ? 'fa-times' : 'fa-bars'} m-2`}
            aria-hidden="true"
            onClick={toggleMenu}
          ></i>
        )}
      ...
      <div className="d-flex">
          ...
          <div>
            <nav className={`nav ${isMobile ? 'show' : ''}`}>
              <div className="m-logo">
                <img className="logo__icon" src="https://www.youshe.id/favicon.ico" alt="Logo" />
                <strong>Edu <span>Hub</span></strong>
              </div>
              <hr />
              {isAdmin && (
                <>
                  <div className="nav__item mt-4" role="button" onClick={showDashboard}>
        ...
        <main className="main">
          ...
          <div className="content-area">
            {!isDashboardVisible && !isUserVisible && !isUserUpdateVisible && !isCourseVisible && (
              <p className="text-center">
                {isAdmin ? (
                  <>
                    Welcome Admin <i className="fa fa-smile-o" aria-hidden="true"></i>
                  </>
                ) : (
                  <>
                    Welcome Student <i className="fa fa-smile-o" aria-hidden="true"></i>
                  </>
                )}
              </p>
            )}
            ...
            {isCourseVisible && <StudentCourseComponent />} {/* Render student course component */}
          </div>
        </main>
      </div>
    </div>
